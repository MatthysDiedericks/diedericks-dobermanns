-- 0073 — Capture trg_contact_dupe_repoint_guard (added live 13 Aug 2026).
--
-- merge_contacts() re-points every FK to contacts(id) in a generic loop. That
-- includes contact_duplicate_candidates.contact_a_id / contact_b_id. When both
-- sides of a pair are re-pointed to the survivor they become equal, violating
-- contact_dupe_ordered and aborting every merge — which broke the duplicate
-- review screen until this guard was added on production.
--
-- The BEFORE UPDATE trigger:
--   * drops the row if a and b become the same contact
--   * swaps a/b so contact_a_id < contact_b_id
--   * drops the row if that ordered pair already exists
--
-- Also exclude contact_duplicate_candidates from the generic FK loop — the
-- function already marks those rows merged explicitly. The trigger stays as
-- defence for any other UPDATE that re-points the pair columns.

create or replace function public.contact_dupe_repoint_guard()
returns trigger
language plpgsql
as $function$
declare
  t uuid;
begin
  if new.contact_a_id = new.contact_b_id then
    delete from public.contact_duplicate_candidates where id = old.id;
    return null;
  end if;

  if new.contact_a_id > new.contact_b_id then
    t := new.contact_a_id;
    new.contact_a_id := new.contact_b_id;
    new.contact_b_id := t;
  end if;

  if exists (
    select 1 from public.contact_duplicate_candidates x
     where x.contact_a_id = new.contact_a_id
       and x.contact_b_id = new.contact_b_id
       and x.id <> old.id
  ) then
    delete from public.contact_duplicate_candidates where id = old.id;
    return null;
...
drop trigger if exists trg_contact_dupe_repoint_guard on public.contact_duplicate_candidates;
create trigger trg_contact_dupe_repoint_guard
  before update on public.contact_duplicate_candidates
  for each row
  execute function public.contact_dupe_repoint_guard();

-- Root-cause fix: do not generic-repoint this table's own pair columns.
