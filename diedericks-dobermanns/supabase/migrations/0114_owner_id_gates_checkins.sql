-- 0114 — Check-ins and follow-ups require a known owner (owner_id).
--
-- 80 sold dogs parked as ownership_status = 'lost_contact' (sold before Feb 2025,
-- buyer never captured) stay on litter / pedigree / progeny records. They must
-- not enter generate_due_check_ins or the Unallocated Sales queue.
--
-- Gate is owner_id, not the dog's age: a 2020 sale whose buyer registers later
-- becomes contactable; a recent sale with no portal owner does not.
--
-- Replaces the 0113 generator, keeping its birthday placeholder-name drafts,
-- and tightens the outer loop from (owner_id OR owner_contact_id) to owner_id.
...
comment on function public.dog_is_contactable(uuid) is
  'True only when a portal owner (owner_id) is linked and contacting is appropriate. Age is not a factor. Unknown id → false.';
...
begin
  ...
  for r in
    ...
    where d.owner_id is not null
      and public.dog_is_contactable(d.id)
  loop
...
comment on function public.generate_due_check_ins(integer) is
  'Creates due check_in rows with drafts for dogs that have a portal owner (owner_id). Age is not used to include or exclude a dog. Birthday drafts never use Puppy/Pup N placeholders. Never sends.';
...
update public.check_ins ci
   set status = 'skipped',
       response_notes = trim(both from coalesce(response_notes, '')
         ...
         || '[auto] Cancelled — no known owner (owner_id) or not contactable'),
       updated_at = now()
 where ci.status = 'due'
   and not public.dog_is_contactable(ci.dog_id);
