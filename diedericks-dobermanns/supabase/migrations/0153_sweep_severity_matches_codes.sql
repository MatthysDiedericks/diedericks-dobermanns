-- Sweep inserts must match CODE_SEVERITY in
--   diedericksdobermann-web/src/lib/errors/codes.ts
--   diedericks-dobermanns/lib/errors/codes.ts
-- That map is the source of truth. Change both.
...
begin
  ...
  -- Severities here must match CODE_SEVERITY in src/lib/errors/codes.ts.
  for r in
    select q.id,
  ...
  loop
    ...
    values (
      'QUOTE_TOTAL_MISMATCH', 'quote', 'critical',
  ...
  begin
    ...
    loop
      insert into public.error_events (code, area, severity, message, detail, surface, actor_role, entity_type, entity_id)
      values (
        'UPLOAD_OBJECT_MISSING', 'upload', 'error',
