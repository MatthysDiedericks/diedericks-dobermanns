...
export function useAdminApplications(): ListResult<Application> {
  return useRemoteList<Application>(MOCK_APPLICATIONS, (client) =>
    client
      .from('applications')
      .select('id, full_name, email, phone, status, purpose, country, created_at, reviewed_at, user_id, admin_notes, archived_at, archived_reason, archived_by, id_type, id_check_status, id_check_note, buyer_location_type, marketing_opt_in, reference_code, merged_into_application_id')
