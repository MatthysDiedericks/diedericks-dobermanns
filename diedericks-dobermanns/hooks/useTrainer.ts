...
const TRAINER_BOOKING_SELECT =
  ...
  'dog:dogs(id, name, colour, dog_media!dog_media_dog_id_fkey(url, thumbnail_url, is_primary, uploaded_at)), ' +
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: err } = await supabase
        .from('training_bookings')
        .select(
          'dog_id, scheduled_at, status, dog:dogs(id, name, colour, status, dog_media!dog_media_dog_id_fkey(url, thumbnail_url, is_primary, uploaded_at))',
        )
        .eq('trainer_id', trainerId)
        .not('dog_id', 'is', null);
      if (err) throw new Error(err.message);
