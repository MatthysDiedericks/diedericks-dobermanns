...
/** Create and edit catalogue items, including shop visibility and product photos. */
export function CatalogueManager({ initialItemId }: { initialItemId?: string }) {
  ...
  useEffect(() => {
    ...
    const it = items.find((row) => row.id === initialItemId);
    if (!it) return;
    setEditingId(it.id);
    setDraft({
      label: it.label,
          ...
          {g.items.map((it) => (
            <Card key={it.id} className={`p-4 ${it.is_active ? '' : 'opacity-50'}`}>
              ...
              <Typography variant="caption" className="mt-1 text-ink-muted">
                {it.item_type.replace(/_/g, ' ')} · {shopPriceLabel(it)} · on {usage[it.code] ?? 0}{' '}
                quotes{it.is_client_visible ? ' · shop' : ' · internal'}
